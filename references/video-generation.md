...
- **Видео опционально** — комбинаторное объявление в любом случае заливается без
  `VideoExtensionIds`: автоматической загрузки видео в MCP нет, ролики
  добавляются к объявлению вручную (см. «Как видео попадает в объявление»).
...
Расширение и вес проверяет `validate_assets.py`, длительность — API Директа
при загрузке.

## Как видео попадает в объявление — вручную

⚠️ **Автоматической заливки видео нет.** `ads_add_responsive` принимает
`video_extension_ids`, но получить эти ID нечем: сервисов `AdVideos` и
`Creatives`, которыми видео загружается и превращается в креатив, в MCP нет —
см. `references/yandex-direct-mcp.md` → «Чего MCP не умеет». Половина цепочки
есть, половины нет.

Поэтому на Шаге 10 комбинаторное объявление заливается **без**
`VideoExtensionIds`, а ролики из `assets/videos/` маркетолог добавляет к
объявлению сам через интерфейс Директа. `preflight.py` предупреждает, если
`combined_ad.videos` непусты, чтобы это всплыло до заливки, а не после.

Пункт обязан попасть в `10_launch_log.md` в список ручной работы: без него
сгенерированные ролики останутся лежать в рабочей папке, и никто о них
не вспомнит.

Полумера, если нужна автоматизация уже сейчас: загрузить видео через интерфейс,
взять готовые ID и положить их в `08_creatives.json` — `ads_add_responsive`
их подхватит.
...
- [ ] Ручная загрузка ролика в объявление внесена в список ручной работы
      `10_launch_log.md` — автоматически видео не заливается
