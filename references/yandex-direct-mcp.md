...
> С версии MCP **0.5.1** оба правила соблюдаются мостом автоматически: без фильтра приходит понятная ошибка **до** обращения к API (баллы не тратятся), длинные `CampaignIds` режутся по 10 и результат склеивается, `adimages_get` без хешей сам делает два вызова. Инструкции выше остаются верными для обеих версий — соблюдая их, ты не ошибёшься ни на 0.5.0, ни на 0.5.1.
...
**Установщик умеет только конфиг Claude Desktop:** он найдёт `claude_desktop_config.json`, сделает бэкап и допишет блок `mcpServers.yandex-direct`. Скажи маркетологу: «закрой Claude Desktop полностью и открой заново, потом вернись и скажи "продолжаем"». **Сам MCP не запускай.**

**Среда другая** (Claude Code, Cursor, своя оболочка) — установщик не применяй: подключение MCP там описывается в собственном формате, и это работа человека, не скилла. Скажи прямо: «MCP `yandex-direct` не подключён, подключение в этой среде делается вручную — продолжаю без live-данных» — и иди по ветке без MCP с пометкой о пропуске.
...
Bun 1.1+ в PATH (Windows: `irm bun.sh/install.ps1 | iex`), папка MCP скачана, `bun install` выполнен, среда с поддержкой MCP (для этого установщика — Claude Desktop 0.7+). Установщик проверит это до правки конфига. Запуск установщика требует способности **запускать код** — её нет, значит и этот путь закрыт: подключение делает человек.
