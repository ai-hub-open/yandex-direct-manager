...
**Обязательно:** `name` (до 255 символов) + **ровно один** источник картинки:
`image_url` (публичная ссылка http/https — сервер Директа скачает файл сам; приватные
адреса и внутренняя сеть отклоняются) **или** `file_path` (абсолютный путь на диске
сервера MCP).
...
Возвращает `AdImageHash` → в `image_hashes` объявления.

✅ **На хостовом MCP картинки заливаются по `image_url`** (сверено живьём 03.09.2026 —
прежнее ограничение «только `file_path`, URL-параметра нет» снято обновлением сервера).
Хост не видит локальные файлы маркетолога, поэтому путь такой: локальный файл →
**KeepImage** (`storage_publish_image` или `scripts/upload_creatives_to_storage.py`) →
публичная ссылка → `adimages_add(image_url=...)`. Ссылка нужна живой только на момент
вызова — Директ скачивает картинку к себе. Полный порядок — SKILL.md → Шаг 10Б и
`mcp-account-integration.md` → Use case 5. `file_path` остаётся для локального
stdio-режима, когда файлы на той же машине, что и сервер.

⚠️ **Только последовательно.** Параллельные вызовы `adimages_add` на MCP дают ошибку
`There are some read requests waiting on finished stream` — грузи картинки по одной.

⚠️ **`5004 Invalid image file type`** — картинка не проходит техтребования Директа
(размеры/формат), это отказ **по контенту**, а не по источнику. Прогоняй
`scripts/validate_assets.py` до заливки.
...
- **файлы маркетолога хосту недоступны** — картинки заливаются по `image_url` через мост KeepImage (см. `adimages_add` и SKILL.md → Шаг 10Б).
