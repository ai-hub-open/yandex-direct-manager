...
⚠️ **Хостовый режим (aihub.click.ru):** `file_path` — это путь на диске **сервера**, файлы маркетолога хосту недоступны. Проверь `tools/list`: если у инструмента появился параметр URL — грузи по публичной ссылке; нет — заливай объявление без картинок и запиши их добавление в `10_launch_log.md` как ручную работу (UI Директа / Коммандер).
...
Позови любой читающий инструмент, например `campaigns_get` с `limit: 1`. Не отвечает или сервера `yandex-direct` нет в сессии — запусти установщик:
...
- **`forecast_bids` недоступен** — Шаг 2 идёт через живой аукцион (`keywordbids_get`) или справочник ниш;
