...
Задай маркетологу **одним блоком** 11 пунктов брифа (продукт, URL, регион, бюджет, каналы, тип запросов, УТП + сужающие, цель в Метрике, CPA-таргет + коллтрекинг, существующие материалы, slug проекта).
...
**Если Wordstat API недоступен** (`python -m scripts.wordstat_api --check` упал или нет ключа) — режим симуляции с пометкой `[SIM]`, в артефакт записываем «реальная проверка не сделана, нужно подключить API до запуска». Сборку семантики маркетолог делает вручную или через сторонние сервисы.
...
- **10-30 ключей** из соответствующего CSV Шага 4
- **3-5 вариантов объявлений:**
  - Заголовок 1 (≤56 символов) — на основе УТП из брифа + слово из запроса
  - Заголовок 2 (≤30 символов) — доказательство или вторая выгода
  - Текст (≤81 символ)
  - **Обязательное вхождение ключевого слова** в один из заголовков
- **4-8 sitelinks** — конкретные фичи продукта
- **2-4 callouts** — короткие уточнения
- `display_url_path` — релевантный URL-path (≤20 символов)
- **Минусы группы** — узкие, только для этой группы

**Тестовая матрица:** минимум 3 объявления на группу с различиями в одной оси (заголовок vs текст vs CTA).
...
**Главное правило:** на старте — **НЕ** «Оптимизация конверсий». «Максимум кликов с лимитом CPC» 2 недели → + «Целевая стоимость клика» → (после 10+ конверсий/нед) → «Оптимизация конверсий».
...
1. **`campaigns_add`** — из `05_campaign_structure.json` со стратегией из `09_bidding_strategy.json`, `counter_id` и `goal_ids` из `07_metrika_goals.json`
   - При `ad_model = "epk_combined"`: текущий MCP умеет только тип `TextCampaign` — **саму ЕПК-кампанию он не создаёт**. Оболочку ЕПК (кампанию/группу) заводим руками (Директ Коммандер/UI) или через passthrough `yandex_direct_api_call`, фиксируем в `10_launch_log.md`. Для `classic` — `campaigns_add` работает как обычно.
2. **`adgroups_add`** — создание групп
3. **`keywords_add`** — заливка ключей
...
5. **Минусы кампании** — через прямой Direct API (`scripts/yandex_direct_api.py`) — MCP не умеет
6. **Sitelinks / Callouts / DisplayUrlPath** — через прямой Direct API (фолбек) или маркетолог руками в UI
7. **Точные ставки в группах** — через прямой Direct API при необходимости тонкого управления

Кампании создаются в статусе **DRAFT**.

**Если MCP-тул не отвечает** — см. `references/yandex-direct-mcp.md` (установка) и при необходимости запусти `scripts/setup_yandex_direct_mcp.py`. Без MCP заливка невозможна; пользователю передаём готовые артефакты для ручной заливки через Direct Commander.
