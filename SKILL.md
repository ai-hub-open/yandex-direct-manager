...
| **Вызов MCP-инструментов** (хостовые серверы `yandex-direct` и `yandex-wordstat`, aihub.click.ru) | Директ: шаги 2, 5, 8, 10. Wordstat: шаги 1, 3 | Шаг 2 — справочник CPC из `references/frequency-calculator.md` вместо живого аукциона; Шаг 5 — регионы по справочнику; Шаг 8 — дедупликация ключей по выгрузке от маркетолога; Шаг 10 — Директ Коммандер по `references/direct-commander-import.md`; Шаги 1 и 3 — `scripts/wordstat_api.py` (нужен ключ Cloud) или браузерный пробив по `references/wordstat-browser.md`. **Теряется:** живые данные аккаунта, автоматическая заливка и автоматический сбор семантики — кампанию и семантику собирает человек руками. |
...
| `references/wordstat-mcp.md` | Шаги 1, 3 + при сбоях Wordstat | **Сбор частотностей и семантики через MCP `yandex-wordstat`:** инструменты, правила волн, фолбеки |
...
- `scripts/wordstat_api.py` — **фолбек** для Шагов 1 и 3, когда MCP `yandex-wordstat` не подключён: клиент Yandex Cloud Search API (Wordstat `topRequests`), `--mode masks` / `--mode semantics` (рекурсивный сбор волнами); env `WORDSTAT_API_KEY` + `WORDSTAT_FOLDER_ID`; `--check` для проверки. Основной путь — MCP, см. `references/wordstat-mcp.md`
...
**Артефакт:** `direct-campaigns/<slug>/00_brief.md` + `_state.json` (current_step, slug, product_name, доступность Wordstat: `wordstat_mcp_available` и/или `wordstat_api_available`).
...
Сбор семантики + кластеризация (Шаг 4) вынесены в изолированный субагент `semantics` (форк): он готовит seed/anchors из `01_masks.json`, собирает семантику волнами через MCP `yandex-wordstat` (фолбек — `scripts.wordstat_api --mode semantics`, нужен ключ Cloud), пишет `03_semantics_raw.csv` и сразу кластеризует его. Тысячи ключей остаются в форке.
...
После проверки масок и расчёта необходимой частотности — собираем полную семантику через **MCP `yandex-wordstat`** (под капотом — Yandex Cloud Search API, метод `topRequests`), в режиме **рекурсивного сбора волнами**. Правила волн, имена инструментов и стоп-условия — в `references/wordstat-mcp.md`; исполняет их субагент `semantics` по `subagents/semantics.md`.
...
Сбор волнами: семена → вызовы инструмента частотностей MCP → найденные запросы с якорем становятся семенами следующей волны; дедупликация и **гарантированная остановка** — по затуханию (волна почти не даёт нового) или по предохранителям. **Профиль «Стандарт» (дефолт):** глубина 2, до 150 семян на волну, порог семени 10 показов, стоп при <5% нового, предохранители ≤1000 вызовов / ≤15 минут. **Профиль «Глубокий»** (если по отчёту мало и нужен ещё хвост): глубина 3, 300 семян на волну, стоп при <3% нового, ≤5000 вызовов / ≤40 минут.

Работай файлами: каждая волна дописывает `03_semantics_raw.csv`, в чат выводятся только итоги волны (1 строка). Тысячи ключей в контекст не тащим.

**Фолбек без MCP** (есть запуск кода и ключ `WORDSTAT_API_KEY` + `WORDSTAT_FOLDER_ID`) — та же логика волн готовым скриптом:
```
...
```
Профиль «Глубокий» у скрипта — флагами `--max-depth 3 --seeds-per-wave 300 --dry-threshold 0.03 --max-api-calls 5000 --max-minutes 40`.
...
⚠️ **Хостовый MCP не видит файлы маркетолога.** `adimages_add` принимает `file_path` на диске **сервера** — для aihub.click.ru это не твоя машина. Порядок действий: (1) проверь в `tools/list`, не появился ли у `adimages_add` параметр URL — появился, грузи по публичной ссылке; (2) нет — заливай объявление **без картинок**, а их добавление запиши в `10_launch_log.md` списком ручной работы (UI Директа или Коммандер). Остальная цепочка на хостовом MCP работает как обычно.
...
|---|---|---|---|
| `semantics` | 3–4 | рекурсивный сбор волнами через MCP `yandex-wordstat` (фолбек `wordstat_api --mode semantics`) → кластеризация по `wordstat-filter.md` на 4 CSV | сводку ≤20 строк + `03_semantics_raw.csv` + `04_keys_*.csv` + `04_wordstat_filter_summary.md` |
