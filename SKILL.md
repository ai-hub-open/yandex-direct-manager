...
Пайплайн опирается на MCP `yandex-direct` (прогноз CPC, дедупликация, заливка). Скилл не проверяет подключение MCP — если на каком-то шаге MCP-тул не отвечает, обрабатываем ошибку по месту (фолбек на справочник CPC, ручную заливку и т.п.).
...
| `references/yandex-direct-mcp.md` | Шаг 10 + при сбоях MCP | Установка MCP-сервера и режимы (sandbox/direct/clickru) |
| `references/mcp-account-integration.md` | Шаг 2 + Шаг 10 | Use cases MCP: аудит существующих кампаний, прогноз CPC, дедупликация, заливка |
| `references/yandex-direct-api.md` | Шаг 10 (фолбек) | Прямой Direct API для sitelinks/callouts |
...
| `references/direct-commander-import.md` | Шаг 10 | Импорт xlsx в Директ Коммандер — заливка комбинаторных РК |
...
- `scripts/generate_ads_xlsx.py` — `creatives.json` → xlsx с валидацией лимитов (или CSV-фолбек)
...
- `scripts/forecast_cpc.py` — оценка CPC по живому аукциону (`KeywordBids.get`): парсит ответ аукциона в base/opt/pess CPC; умеет звать API сам при `YANDEX_DIRECT_TOKEN`
...
- `scripts/yandex_direct_api.py` — прямой Direct API (фолбек для sitelinks/callouts)
...
Задай маркетологу **одним блоком** 12 пунктов брифа (продукт, URL, регион, бюджет, каналы, тип запросов, УТП + сужающие, цель в Метрике, CPA-таргет + коллтрекинг, существующие материалы, slug проекта, **лицензируемая/запрещённая тематика + документы**).
...
**Прочти `references/frequency-calculator.md` целиком.** Параллельно — Use case 1 и 2 из `references/mcp-account-integration.md` (аудит существующих кампаний в аккаунте и `keywords_get` для дедупликации/CPC-прогноза).
...
**Приоритет источника CPC:** историческая AvgCpc похожих кампаний (`report_campaign`) → живой аукцион `KeywordBids.get` (парсит `scripts/forecast_cpc.py`) → справочник CPC по нишам → override от маркетолога. Сам у маркетолога CPC не спрашиваем. Метода `Forecast.GetForecast` в API v5 нет — он закрыт.
...
После — привязка `counter_id` и `goal_ids` к кампаниям через MCP yandex-direct (на Шаге 10).
...
1. Дедупликация ключей через `keywords_get` (Use case 2А в `mcp-account-integration.md`)
...
После — `python -m scripts.generate_ads_xlsx --workspace <path>` → xlsx/csv + `warnings.txt`. Скрипт валидирует лимиты комбинаторного (≤7 заголовков, ≤3 текстов, ≤5 картинок, ≤6 видео).
...
- `09_bidding_strategy.json` — machine-readable для заливки
...
✅ region_ids (Use case 3 в mcp-account-integration.md)
```
...
См. `references/mcp-account-integration.md` Use case 5:

Модель одна — комбинаторное объявление, которое текущий MCP (`ads_add` умеет только `TextAd`) не создаёт. Заливка комбинаторного **штатно поддерживается через XLS/XLSX, Директ Коммандер и API** (№50). Поэтому два трека:

**Трек A — Директ Коммандер (основной для комбинаторных РК).** По `ads.xlsx`/`keywords.xlsx` Шага 8 импортируем всю кампанию (кампания + группа + комбинаторное объявление + ключи) в Директ Коммандер и отправляем в **DRAFT**. Пошагово — `references/direct-commander-import.md`. Догрузка минус-фраз/быстрых ссылок/ставок — в самом Коммандере.

**Трек Б — MCP/API (для того, что умеют).** См. `references/mcp-account-integration.md` Use case 5:
1. **`campaigns_add`** — MCP умеет только `TextCampaign`; **ЕПК-оболочку он не создаёт**. Оболочку ЕПК заводим через Коммандер (Трек A) или passthrough `yandex_direct_api_call`, фиксируем в `10_launch_log.md`.
2. **`adgroups_add`** / **`keywords_add`** — группы и ключи, где применимо.
3. **`ads_add`** — комбинаторное объявление НЕ создаётся (только `TextAd`) → идёт через Трек A.
4. **Минусы кампании** — прямой Direct API (`scripts/yandex_direct_api.py`) или Коммандер.
5. **Sitelinks / Callouts / DisplayUrlPath / точные ставки** — прямой Direct API или Коммандер/UI.

Кампании создаются в статусе **DRAFT**. Всё, что не удалось залить автоматически, — списком «доделать руками» в `10_launch_log.md`.

**Если MCP-тул не отвечает** — см. `references/yandex-direct-mcp.md` (установка) и при необходимости запусти `scripts/setup_yandex_direct_mcp.py`. Основной путь заливки комбинаторного и без MCP — **Директ Коммандер по файлам Шага 8** (`references/direct-commander-import.md`).
...
08_creatives.json ──→ Шаг 10 (заливка объявлений; комбинаторное объявление — через Директ Коммандер по xlsx Шага 8)
