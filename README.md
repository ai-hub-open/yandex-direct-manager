...
- создаёт кампанию в черновике через MCP: оболочку, группу, комбинаторное объявление, быстрые ссылки, уточнения, фразы и ставки;
- собирает медиаплан и таблицу объявлений, а при недоступном MCP — файлы для загрузки через Директ Коммандер.
...
10. **Визуалы.** Генерация изображений и коротких видео для РСЯ и комбинаторных объявлений — при заданных ключах сервисов генерации; без ключей агент готовит техническое задание. Загрузка готовых файлов в кабинет остаётся ручной: MCP-сервер читает файлы только со своего диска.
